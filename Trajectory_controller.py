...
# --- ALIAS CHO ENUM TƯƠNG ỨNG ---
CLOSED_LOOP_CONTROL = AXIS_STATE_CLOSED_LOOP_CONTROL
IDLE = AXIS_STATE_IDLE
TORQUE_CONTROL = CONTROL_MODE_TORQUE_CONTROL
FULL_CALIBRATION = AXIS_STATE_FULL_CALIBRATION_SEQUENCE
...
class ODriveThread(threading.Thread):
    ...
    # --- HÀM MỚI: DÙNG CHO NÚT CALIB TỪ GUI ---
    def run_calibration(self):
        try:
            if self.connected and self.axis is not None:
                print("Đang xóa lỗi cũ (nếu có)...")
                self.clear_error()   # Dọn dẹp lỗi cũ trước khi calib
                time.sleep(0.1)      # Dừng 1 chút để board ODrive kịp xử lý
                
                print("Đang tiến hành Calibration...")
                # Nếu ông dùng alias tôi báo ở trên thì gọi FULL_CALIBRATION
                # Hoặc viết tường minh như GUI của ông là: AXIS_STATE_FULL_CALIBRATION_SEQUENCE
                self.axis.requested_state = FULL_CALIBRATION 
            else:
                print("Cannot calibrate: ODrive not connected.")
        except Exception as e:
            print("Lỗi calib:", e)
    ...
    def enter_closed_loop(self):
        self.clear_error()
        # Set control mode sang Torque Control
        self.axis.controller.config.control_mode = TORQUE_CONTROL 
        self.axis.requested_state = CLOSED_LOOP_CONTROL
        self.closed_loop_control = True
        print("Entered CLOSED_LOOP_CONTROL mode (Torque Control).")
